...
export default function ManajemenKamarPage(){
  ...
  const fetchData = async ()=>{ setLoading(true); try{ const res = await axios.get(`${API_BASE}/kamar`); setData(res.data) }catch(e){ console.error(e); toast({ title: 'Gagal', description: 'Gagal memuat kamar', variant:'destructive' }) } finally{ setLoading(false) } }
  useEffect(()=>{ fetchData() }, [])
        ...
        <Card><CardContent>{loading ? <div>Memuat...</div> : (
          <div>
            <div className="flex justify-between items-center mb-4"><div/> <Dialog open={addOpen} onOpenChange={setAddOpen}><DialogTrigger asChild><Button>Tambah Kamar</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Tambah Kamar</DialogTitle><DialogDescription>Isi informasi kamar</DialogDescription></DialogHeader>
              <form onSubmit={addForm.handleSubmit(async (vals)=>{ try{ const payload = { ...vals, kapasitas: Number(vals.kapasitas) || 0 }; await axios.post(`${API_BASE}/kamar`, payload); await fetchData(); toast({ title: 'Berhasil', description: 'Kamar ditambahkan' }); addForm.reset(); setAddOpen(false) }catch(e:any){ console.error(e); toast({ title: 'Gagal', description: e?.response?.data?.message || 'Gagal menambahkan', variant:'destructive' }) } })} className="grid grid-cols-2 gap-4 py-4">
