...
type Indikator = { id:number; nama_indikator:string; is_active?: boolean }
type FormValues = { nama_indikator: string; is_active?: boolean }
...
export default function ManajemenIndikatorSikapPage(){
  ...
  const form = useForm<FormValues>({ defaultValues: { nama_indikator: '', is_active: true } })
  const addForm = useForm<FormValues>({ defaultValues: { nama_indikator: '', is_active: true } })
  ...
  const handleAdd = async (vals: FormValues) => {
    try {
      await axios.post(`${API_BASE}/indikator-sikap`, vals);
      await fetchData();
  ...
  const handleEdit = async (vals: FormValues) => {
    if (!editing) return;
    try {
      await axios.put(`${API_BASE}/indikator-sikap/${editing.id}`, vals);
      await fetchData();
      setEditing(null);
  ...
  const handleDelete = async () => {
    if (!deleting) return;
    try {
      await axios.delete(`${API_BASE}/indikator-sikap/${deleting.id}`);
      await fetchData();
      setDeleting(null);
  ...
  const columns: ColumnDef<Indikator, any>[] = [
    { header: 'Nama Indikator', accessorKey: 'nama_indikator' },
      ...
      <div className="space-y-6">
          ...
          <CardContent>
            {loading ? <div>Memuat...</div> : (
              ...
              <form onSubmit={addForm.handleSubmit(handleAdd)} className="grid grid-cols-2 gap-4 py-4">
                <div className="space-y-2"><Label htmlFor="nama_indikator">Nama Indikator</Label><Input id="nama_indikator" {...addForm.register('nama_indikator',{ required: true })} /></div>
                ...
                <DataTable<Indikator> columns={columns} data={data} onEdit={(r)=>{ setEditing(r); form.reset({ nama_indikator: r.nama_indikator, is_active: r.is_active }) }} onDelete={(r)=>setDeleting(r)} />
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Dialog open={!!editing} onOpenChange={(v)=>{ if(!v) setEditing(null) }}>
          <DialogContent>
            ...
            <form onSubmit={form.handleSubmit(handleEdit)} className="grid grid-cols-2 gap-4 py-4">
              <div className="space-y-2"><Label htmlFor="edit-nama_indikator">Nama Indikator</Label><Input id="edit-nama_indikator" {...form.register('nama_indikator',{ required: true })} /></div>
        ...
        <Dialog open={!!deleting} onOpenChange={(v)=>{ if(!v) setDeleting(null) }}>
          <DialogContent>
            ...
            <div className="py-2">Apakah Anda yakin ingin menghapus indikator <strong>{deleting?.nama_indikator}</strong> ?</div>
