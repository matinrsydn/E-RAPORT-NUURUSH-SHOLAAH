...
type NilaiUjian = { id:number; siswa_nis?:string; mapel_nama?:string; nilai?:number }

export default function ManajemenNilaiUjianPage(){
  const [data, setData] = useState<NilaiUjian[]>([])
  const [loading, setLoading] = useState(true)
  const [editing, setEditing] = useState<NilaiUjian | null>(null)
  const [deleting, setDeleting] = useState<NilaiUjian | null>(null)
  ...
  const fetchData = async ()=>{
    ...
    try{
      ...
      } else toast({ title: 'Gagal memuat data', description: String(e), variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const fetchOptions = async ()=>{
    try{
      ...
      } else toast({ title: 'Gagal memuat opsi', description: String(e), variant: 'destructive' })
    }
  }
  ...
  const columns: ColumnDef<NilaiUjian, any>[] = [
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div><h1 className="text-3xl font-bold">Manajemen Nilai Ujian</h1><p className="text-muted-foreground">Kelola nilai ujian siswa</p></div>
        <Card>
          <CardContent>
            {loading ? <div>Memuat...</div> : (
              <div>
                <div className="flex justify-between items-center mb-4"><div/> <Dialog open={addOpen} onOpenChange={setAddOpen}><DialogTrigger asChild><Button>Tambah</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Tambah Nilai Ujian</DialogTitle></DialogHeader>
        <form onSubmit={addForm.handleSubmit(handleAdd)} className="grid grid-cols-2 gap-4 py-4">
                ...
                <DataTable<NilaiUjian> columns={columns} data={data} onEdit={(r)=>{ setEditing(r); form.reset({ siswa_id: (r as any).siswa_id || 0, mapel_id: (r as any).mapel_id || 0, nilai: r.nilai || 0 }) }} onDelete={(r)=>setDeleting(r)} />
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Dialog open={!!editing} onOpenChange={(v)=>{ if(!v) setEditing(null) }}>
          <DialogContent>
            <DialogHeader><DialogTitle>Edit Nilai Ujian</DialogTitle></DialogHeader>
